...
function create(Quill) {
  ...
  class VideoBuilder {
    ...
    setIframeDimensions(w, h) {
      this.iframe.width = Math.round(w);
      this.iframe.height = Math.round(h);
    }
    ...
    handleDrag(event){
      ...
      if (this.dragBox === this.boxes[0] || this.dragBox === this.boxes[2]) {
        var w = this.preDragWidth - deltaX;
        var h = this.preDragHeight + deltaY;
      }
      else {
        var w = this.preDragWidth + deltaX;
        var h = this.preDragHeight + deltaY;
      ...
      this.node.format('width', w);
      this.node.format('height', h);
      this.setIframeDimensions(w, h);
    }
